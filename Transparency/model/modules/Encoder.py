...
from Transparency.model.modules.orthobilstm import OrthoBiLSTM
...
class EncoderorthoRNN(Encoder) :
    def __init__(self, vocab_size, embed_size, hidden_size, pre_embed=None) :
        ...
        self.hidden_size = hidden_size
        self.rnn = OrthoBiLSTM(LSTMCell, input_size=embed_size, hidden_size=2*hidden_size, num_layers=self.num_layers, batch_first=True)
        # make sure decoder expects the right hidden size. In BiLSTM hidden dim is doubled due to concatenation of hidden states.
        self.output_size = self.hidden_size * 2 * 2
        print('ortho-bi-LSTM definition:')
        self.numTrainableParameters()
